...
APP_VERSION = "5.6"     # Phiên bản cứng của bản Build này
...
# Định tuyến thư mục chạy ứng dụng – lấy đúng thư mục chứa .exe hoặc .py
if getattr(sys, 'frozen', False):
    application_path = os.path.dirname(sys.executable)
else:
    application_path = os.path.dirname(os.path.abspath(__file__))

# Ép thư mục làm việc về đúng vị trí đó
os.chdir(application_path)

# Đọc cấu hình local nếu Admin đã từng lưu
_cfg_path = os.path.join(application_path, "app_config.json")
    ...
    def _load_cloud_config(self):
        ...
        def fetch():
            ...
            try:
                ...
                my_ver = APP_VERSION
                cloud_ver = data.get("version")
                update_url = data.get("update_url", "")
                excel_url  = data.get("excel_url", "")

                # So sánh version đúng chuẩn: chỉ cho chạy khi Cloud và app khớp tuyệt đối.
                def _parse_ver(v):
                    try: return tuple(int(x) for x in str(v).split('.'))
                    except: return (0,)
                if cloud_ver is None or _parse_ver(cloud_ver) != _parse_ver(my_ver):
                    remote_ver = cloud_ver if cloud_ver is not None else "không xác định"
                    msg = data.get("thong_bao", f"Phiên bản hiện tại là {my_ver}, nhưng GitHub đang yêu cầu {remote_ver}. Vui lòng cập nhật bản mới để dùng tiếp!")
                    self.win.after(0, lambda m=msg, u=update_url, e=excel_url: self._show_update_block(m, u, e))
                    return

                # ── Tự động tải lophoc.xlsx nếu chưa có cạnh exe ──
                if excel_url and getattr(sys, 'frozen', False):
                    excel_local = os.path.join(application_path, 'lophoc.xlsx')
                    if not os.path.exists(excel_local):
                        try:
                            with requests.get(excel_url, headers={'User-Agent': 'ULSA/1.0'},
                                              stream=True, timeout=30, verify=False) as rx:
                                rx.raise_for_status()
                                with open(excel_local, 'wb') as fx:
                                    for chunk in rx.iter_content(chunk_size=8192):
                                        if chunk: fx.write(chunk)
                        except Exception:
                            pass  # Im lang - khong crash app neu offline

                # Reload QR neu Ma Sinh Vien da nhap (de update so tien moi)
                masv = self.masv_var.get().strip()
                if len(masv) >= 5:
                    self.win.after(0, lambda: self._on_masv_change(None))

            except Exception as ex:
                error_msg = f"Không thể xác minh phiên bản từ GitHub: {ex}\nVui lòng kiểm tra mạng hoặc cập nhật cấu hình cloud."
                self.win.after(0, lambda m=error_msg: self._show_update_block(m, "", ""))
            
        threading.Thread(target=fetch, daemon=True).start()

    def _show_update_block(self, msg, update_url, excel_url=""):
        if not update_url:
        ...
        def do_update():
            ...
            def download_thread():
                ...
                try:
                    ...
                    new_exe  = os.path.join(exe_dir, "__update_temp.exe")
                    ...
                    # Tải file lịch học mới nếu có excel_url
                    if excel_url:
                        self.win.after(0, lambda: lbl_status.config(text="Đang tải file lịch học (lophoc.xlsx) mới..."))
                        try:
                            target_excel = os.path.join(exe_dir, "lophoc.xlsx")
                            with requests.get(excel_url, headers={'User-Agent': 'ULSA/1.0'}, stream=True, timeout=30, verify=False) as rexcel:
                            ...
                            pass  # Không tải được Excel thì bỏ qua, không chặn update exe
...
del "%~f0"'''
    ...
    def _build_ui(self):
        ...
        self.transfer_note_var = tk.StringVar(value="ULSA")
    ...
    def _on_masv_change(self, event=None):
        ...
        masv = self.masv_var.get().strip()
        if len(masv) < 3:
            self.transfer_note_var.set("ULSA")
        ...
        self.transfer_note_var.set(noi_dung)
    ...
    def _check_payment(self):
        ...
        def _normalize_tx_content(value: str) -> str:
        ...
        def run_check():
            try:
                ...
                found_valid = False
                masv_digits = _digits_only(masv)
                
                for tx in transactions:
                    # SePay v2 dùng amount_in và transaction_content
                    amount  = float(tx.get("amount_in", 0) or tx.get("amount", 0))
                    content = _normalize_tx_content(tx.get("transaction_content") or tx.get("description") or "")
                    content_digits = _digits_only(content)

                    exact_match = masv in content
                    
                    if (amount >= GIA_TIEN and 
                        "ULSA" in content and
                        (exact_match or masv_digits == content_digits or masv_digits in content_digits)):
                ...
                if not found_valid:
                    ...
                    self.win.after(0, lambda: messagebox.showinfo(
                        ...
                        f"ULSA {masv}\n\n"
                            f"ℹ️ Nếu bạn đang gia hạn (tài khoản hết hạn), vui lòng chuyển khoản với nội dung \"ULSA {masv}\" sau đó bam nút kiểm tra lại.\n\n"
...
class CompleteGUI:
    ...
    def setup_config_tab(self):
        ...
        self.excel_path_entry.insert(0, os.path.join(application_path, 'lophoc.xlsx'))
    ...
    def _normalize_for_match(self, text):
        """Chuẩn hóa chuỗi để so khớp: bỏ dấu tiếng Việt, bỏ ký tự đặc biệt, uppercase."""
        if text is None:
            return ""
        ...
        # Chuẩn hóa chữ Đ/đ trước khi bỏ dấu.
        txt = txt.replace("Đ", "D").replace("đ", "d")
        txt = unicodedata.normalize("NFD", txt)
        txt = "".join(ch for ch in txt if unicodedata.category(ch) != "Mn")
        return re.sub(r"[^A-Za-z0-9]", "", txt).upper()

    def _extract_class_search_target(self, line):
        """Lấy cụm giống mã lớp nhất trong một dòng dán (hỗ trợ copy từ Excel bảng)."""
        if not line:
            return ""
        ...
        for token in tokens:
            ...
            has_sep = ("_" in t) or ("." in t) or ("-" in t)
            has_level = bool(re.search(r"\b(LT|TH|BT)\b", t.upper()))
            if has_sep or has_level:
                candidates.append(t)

        if candidates:
            # Ưu tiên token dài nhất vì thường đầy đủ thông tin lớp hơn.
    ...
    def parse_and_add_courses(self):
        ...
        try:
            ...
            all_classes = [c for c in df['Tên lớp tín chỉ'].dropna().unique().tolist() if isinstance(c, str)]
            class_entries = []
            for cls in all_classes:
                cls_clean = self._normalize_for_match(cls)
                if not cls_clean:
                    continue
                cls_prefix_raw = cls.split('_')[0] if '_' in cls else cls
                cls_prefix_clean = self._normalize_for_match(cls_prefix_raw)
                class_entries.append((cls, cls_clean, cls_prefix_clean))
            ...
            for line in lines:
                ...
                search_target = self._extract_class_search_target(line)

                self.log_message(f"🔍 Phân tích dòng: '{line}'")
                
                # Chuẩn hóa để PHU/PHỤ được coi là tương đương khi so khớp.
                input_clean = self._normalize_for_match(search_target)
                ...
                input_prefix_raw = search_target.split('_')[0] if '_' in search_target else search_target
                input_prefix_clean = self._normalize_for_match(input_prefix_raw)
                
                # ── Thuật toán tìm kiếm uu tiên mã môn trước ──
                # Bước 1: Tìm chính xác 100% (case-insensitive, bỏ kyï tự đặc biệt)
                for cls, cls_clean, _ in class_entries:
                ...
                if not best_cls:
                    # Bước 2: So khớp mờ nhưng ưu tiên mạnh theo prefix mã môn.
                    for cls, cls_clean, cls_prefix in class_entries:
                        ...
                        # Tăng điểm nếu một chuỗi chứa chuỗi kia (copy thiếu/thừa vài đoạn).
                        if input_clean in cls_clean or cls_clean in input_clean:
                        ...
                        if input_prefix_clean and cls_prefix:
                            if input_prefix_clean == cls_prefix:
                                ...
                                # Prefix lệch hoàn toàn thì trừ điểm để giảm map sai môn.
                                prefix_bonus = -15
                ...
                if best_cls and best_score >= 60:
                    ...
                    if input_clean != self._normalize_for_match(best_cls):
    ...
    def load_existing_config(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                # Luôn trỏ về lophoc.xlsx cạnh file exe (bỏ qua đường dẫn lưu trong config)
        ...
        def _run_env_checks_thread():
            try:
                ...
                try:
                    ...
                    def _get_chrome_ver():
                        # 1. Kiểm tra registry HKEY_LOCAL_MACHINE (cài system-wide)
                        for key_path in [r"SOFTWARE\Google\Chrome\BLBeacon", r"SOFTWARE\WOW6432Node\Google\Chrome\BLBeacon"]:
                            try:
                                ...
                                pass
                        # 2. Kiểm tra registry HKEY_CURRENT_USER (cài per-user)
                        for key_path in [r"SOFTWARE\Google\Chrome\BLBeacon", r"SOFTWARE\WOW6432Node\Google\Chrome\BLBeacon"]:
                            try:
                                key = _winreg.OpenKey(_winreg.HKEY_CURRENT_USER, key_path)
                                ver, _ = _winreg.QueryValueEx(key, "version")
                                _winreg.CloseKey(key)
                                return ver
                            except Exception:
                                pass
                        # 3. Kiểm tra đường dẫn cài đặt phổ biến
                        ...
                        for c_path in chrome_paths:
                            if c_path and os.path.exists(c_path):
                                return "đã cài"
                        # 4. Dùng shutil.which để tìm chrome trong PATH
                        import shutil
