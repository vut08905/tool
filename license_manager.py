...
def get_real_time() -> datetime:
    """Lấy thời gian thực từ internet để chống gian lận chỉnh giờ máy.
    Nếu không có mạng, dùng giờ máy tính."""
    try:
        url = "http://worldtimeapi.org/api/timezone/Asia/Ho_Chi_Minh"
        with urllib.request.urlopen(url, timeout=4) as resp:
            data = json.loads(resp.read().decode())
            dt_str = data["datetime"][:19]           # "2026-03-26T16:00:00"
            return datetime.strptime(dt_str, "%Y-%m-%dT%H:%M:%S")
    except Exception:
        # Không có mạng → dùng giờ máy tính
        return datetime.now()
