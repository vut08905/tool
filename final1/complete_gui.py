...
APP_VERSION = "2.7"     # Phiên bản cứng của bản Build này
...
class CompleteGUI:
    ...
    def start_automation(self):
        ...
        # ── Kiểm tra bản quyền Online trước khi chạy ──
        res = license_manager.check_license()
        if not res.get("valid", False):
            ...
            self.root.withdraw()
            from final.complete_gui import LicenseWindow
            def on_reactivated(masv: str):
                ...
                for widget in self.root.winfo_children():
                    widget.destroy()
                from final.complete_gui import CompleteGUI
                new_app = CompleteGUI(self.root, licensed_masv=masv, expire_date=exp, days_left=days)
